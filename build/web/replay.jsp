<%-- 
    Document   : create
    Created on : Dec 9, 2015, 5:26:32 PM
    Author     : th
--%>

<%@page contentType="text/html" pageEncoding="UTF-8"%>
<!DOCTYPE html>
<!DOCTYPE html>
<html>
<head>
        <meta charset="UTF-8">
        <meta name="viewport" content="width=device-width, initial-scale=1.0">
	<title>Email</title>
	<link rel="stylesheet" type="text/css" href="style.css" />
</head>
<body>
	<% 
            String mail=request.getSession().getAttribute("session_mail").toString();
            String mail_ID = request.getParameter("ID");
            String subject = request.getParameter("subject");
            String sender = request.getParameter("sender");
            
        %>
        <div><img class ="image" src ='image_1.jpg'></div>
        
        <div><h3>Welcome , <%=mail%></h3></div>
        
        <div id="menu">
            <ul>
                <li><a href="sendMessege.jsp">Send message</a></li>
		<li><a href="editProfile.jsp">Edit profile</a></li>
                <li><a href="logout">Logout</a></li>
            </ul>
        </div>
        
        <div id="sideMenu">
            <ul>
                <li><a href="userProfile.jsp">Inbox</a></li>
                <li><a href="sendMessege.jsp">Sent</a></li>
                <li><a href="archiveMessages.jsp">Archive</a></li>
            </ul>
        </div>
        
        <div id="content1">
            <form action = "replay">
                <input type="hidden" name="receiver" value="<%=sender%>"/>
                <input type="hidden" name="subject" value="<%=subject%>" /><br>
                <input type="hidden" name="mail_id" value="<%=mail_ID%>" /><br>
                <textarea rows="10" cols="102" name="content" placeholder="Reply content"></textarea>
                <center><input class ="button" type="submit" value="Send" name="send" /></center>
            </form>
	    </div>
</body>
</html>